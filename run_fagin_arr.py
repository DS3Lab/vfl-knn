...
from utils.fagin_utils import count_by_arr, suggest_size
...
def run(args):
    ...
    for i in range(args.n_test):
        ...
        while cur_n_top < args.k and send_ind <= n_data:

            gather_start = time.time()
            new_lists = gather(local_dist_ind[send_ind:min(n_data, send_ind+send_size)])
