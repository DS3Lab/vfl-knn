...
class LRTrainer(object):
    ...
    def one_iteration(self, n_epoch, n_iter, X, Y):
        ...
        encrypted_Z = self.encrypt_data(partial_Z_flat, self.n_threads)
        ...
        Z = np.asarray(self.global_data).reshape(partial_Z.shape)
        ...
        print("iteration[{}] finish, cost {:.2f} s, encryption cost {:.2f} s, server comm cost {:.2f} s, "
              "grad = {}, loss = {:.6f}, weight = {}"
              .format(n_iter, iter_time, encr_time, trans_time, grad, loss, self.W))
