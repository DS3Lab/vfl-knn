...
class FaginArrTrainer(object):
    ...
    def find_top_k(self, test_data, test_target, k, group_flags):
        ...
        if rank == 0:
            ...
            bc_time += time.time() - bc_start
        sync_top_k_time = time.time() - sync_top_k_start

        # comm top-k local distance
        top_k_dist_start = time.time()
        top_k_local_dist = local_dist[top_k_ids]
        is_attend = group_flags[self.args.rank]
        if is_attend == 0:
            top_k_local_dist = np.zeros_like(top_k_local_dist)
        top_k_dist = sum_sqrt_all_reduce(top_k_local_dist)
        top_k_dist_time = time.time() - top_k_dist_start

        # select top-k
        select_top_start = time.time()
        ind_k = np.argsort(top_k_dist)[:self.args.k]
        top_k_ids = np.asarray(top_k_ids)[ind_k]
        sorted_dist_top_k = top_k_dist[ind_k]
        ...
        print("find top-k finish: target = {}, prediction = {}, total cost {:.2f} s, "
              ...
              .format(test_target, pred_target, time.time() - start_time,
                      ...
                      sync_top_k_time, top_k_dist_time, select_top_time, count_label_time))
