...
class FaginArrTrainer(object):
    ...
    def transmit(self, q, i):
        ...
        print("server {}, addr: {}, splitSize:{}". format(i, addr, split_size))
    ...
    def find_top_k(self, test_data, test_target, k):
        ...
        while cur_n_top < self.args.k and send_ind <= self.n_data:
            ...
            if rank == 0:
                ...
                coordinator_count_by_arr(new_lists, counts, top_k_ids, self.args.k)
        ...
        if rank == 0:
            ...
            cur_n_top = len(top_k_ids)
            dist.broadcast(torch.tensor(cur_n_top), 0)
            top_k_tensor = torch.tensor(top_k_ids, dtype=torch.int32)
            ...
            dist.broadcast(tmp_tensor, 0)
            top_k_ids = tmp_tensor.tolist()
            bc_time += time.time() - bc_start

        max_msg_size = 80000000
        options = [('grpc.max_send_message_length', max_msg_size), ('grpc.max_receive_message_length', max_msg_size)]
        if rank != 0:
            # reindex top-k indices
            reindex_start = time.time()
            reindex_top_k_ids = self.shuffled2original[top_k_ids]
            reindex_time = time.time() - reindex_start

            # select local distances of k
            top_k_local_dist = local_dist[reindex_top_k_ids[:self.args.k]]
            ...
            encr_local_top_k_dist = self.encrypt_data(top_k_local_dist, self.n_threads)
        ...
        if rank != 0:
            global_top_k_sort_ind = np.argsort(global_top_k_dist)[:k]
            global_top_k_ids = reindex_top_k_ids[global_top_k_sort_ind]
            print("indices of top-{} near neighbors = {}".format(k, global_top_k_ids))
            print("distance of top-{} near neighbors = {}".format(k, global_top_k_dist[global_top_k_sort_ind]))
            ...
            print("find top-k finish: target = {}, prediction = {}, cost {:.2f} s, "
                  ...
                  .format(test_target, pred_target, time.time() - start_time,
                          ...
                          homo_time, reindex_time, encr_time, split_time, trans_time, count_label_time))
