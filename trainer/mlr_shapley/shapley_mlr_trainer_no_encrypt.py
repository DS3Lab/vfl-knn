...
class ShapleyMLRTrainer(object):
    ...
    def __init__(self, args, group_flags):
        ...
        # init local weight
        self.n_class = self.args.n_classes
        self.W = np.zeros((self.n_f, self.n_class), dtype=np.float32)
    ...
    def one_iteration(self, n_epoch, n_iter, X, Y):
        ...
        partial_Z = - X @ self.W if self.is_attend == 1 else np.zeros((X.shape[0], self.n_class))
        ...
        #      .format(n_iter, iter_time, comm_time, self.is_attend, grad, loss, self.W))
    ...
    def predict_one(self, X):
        ...
        partial_Z = - X @ self.W if self.is_attend == 1 else np.zeros(self.n_class)
        comm_dot_start = time.time()
...
if __name__ == '__main__':
    ...
    for i in range(10):
        Z = - X @ W
        print("Z: {}, shape {}".format(Z, Z.shape))
        P = softmax(Z, axis=1)
        print("P: {}, shape {}".format(P, P.shape))
        N = X.shape[0]
