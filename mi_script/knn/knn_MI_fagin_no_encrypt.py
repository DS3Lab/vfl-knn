...
from mi_trainer.knn.fagin_trainer_no_encrypt import FaginArrTrainer
...
def run(args):
    ...
    for key_ind in range(len(sampled_group_key)):
        group_key = sampled_group_key[key_ind]
        ...
        avg_candidate = int(trainer.n_cur_fagin_cand / args.n_test)
        trainer.n_cur_fagin_cand = 0

        print("group {} finish: test {} data cost {} s, avg # candidate = {}, mutual info of clients = {}"
              .format(group_flags, args.n_test, time.time() - group_start, avg_candidate, client_mi_values))

    mi_sort_ind = np.argsort(client_mi_values)[::-1]
    print("{} group test cost {:.2f} s, avg # candidate = {}, client mi ranking = {}, values = {}"
          .format(len(sampled_group_key), time.time() - test_start,
