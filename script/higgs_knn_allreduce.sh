...
for ((i=0; i<$world_size; i++)); do
  ...
  if [[ $i == 0 ]]
	then
	  ...
	  mkdir -p $dir_path
    rm -f $log_path
	  nohup python -u higgs_knn_allreduce.py --init-method tcp://$master_ip --rank 0 --world-size $world_size \
    ...
    ssh $node_name "source $env_path; cd $code_path; nohup python -u higgs_knn_allreduce.py --init-method tcp://$master_ip --rank $i --world-size $world_size --root $root --n-features $n_features --n-classes $n_classes --k $k --n-test $n_test --no-cuda > $log_path 2>&1 &"
  fi
done